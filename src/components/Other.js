...
const Other = () => {
  ...
  return (
    <div className='px-2 md:px-4 md:py-4'>
      <h1 className='text-2xl mt-4 font-bold'>Other Projects</h1>
            ...
            {frontendProjectsData.map(project => (
              <div key={project.id} className="w-40 md:w-52 bg-white min-h-48 md:min-h-[220px] max-h-fit rounded-md overflow-hidden relative cursor-pointer">
                <div onClick={(e) => openProjectLink(e, project.link)} className="rounded h-28 md:h-36 shadow-lg">
                  <img className="object-cover w-[100%] h-[100%] md:w-full md:h-full" src={project.imageUrl} alt={project.title} loading="lazy" />
